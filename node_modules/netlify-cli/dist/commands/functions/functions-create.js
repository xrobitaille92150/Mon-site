...
const isValidFunctionName = (name) => typeof name === 'string' && /^[\w.-]+$/i.test(name);
const validateFunctionName = (name) => {
    if (!isValidFunctionName(name)) {
        throw new Error(`Invalid function name "${String(name)}". Function names must only contain letters, numbers, hyphens, underscores, or dots.`);
    }
};
/**
...
const pickTemplate = async function ({ language: languageFromFlag, template: templateFromFlag }, funcType) {
    ...
    if (templateFromFlag) {
        ...
        if (!match) {
            return logAndThrowError(`Template "${templateFromFlag}" not found for language "${language}". Run \`netlify functions:create\` without --template to browse available templates.`);
        }
        return match.value;
    }
...
const getSafeFunctionPath = (functionsDir, name) => {
...
const resolveTemplateMetadata = async (templateName, languageHint) => {
    const langs = languageHint
        ? [languageHint]
        : languages.map((lang) => lang.value).filter(Boolean);
...
export const functionsCreate = async (name, options, command) => {
    ...
    if (typeof options.template === 'string') {
        ...
        if (!resolved) {
            return logAndThrowError(`Template "${options.template}" not found${options.language ? ` for language "${options.language}"` : ''}.`);
        }
