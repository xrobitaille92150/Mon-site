...
const renderPretty = (params) => {
    const { enabled, packageInstalled, connectionString, showCredentials, status, isLocal, hasUrlOverride, migrationsDirectory, projectRoot, adminUrl, } = params;
    ...
    log(chalk.gray(`${INDENT}These migrations have been applied and cannot be edited or deleted. Any changes to the schema must involve a new migration.`));
    ...
    log(chalk.gray(`${INDENT}These migrations are defined locally but haven't been applied, and you can change them or delete them.`));
