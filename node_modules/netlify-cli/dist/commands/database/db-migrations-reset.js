...
export const migrationsReset = async (options, command) => {
...
const resetAgainstLocal = async (json, command) => {
...
const resetAgainstBranch = async (branch, json, command) => {
    ...
    const migrationsDirectory = resolveMigrationsDirectory(command);
    if (!json) {
        log(`Removing local migration files that have not been applied to database branch ${chalk.bold(branch)}. ` +
...
const logOutcome = (deleted, params) => {
    if (params.json) {
        logJson({
            reset: true,
...
const deletePendingMigrationFiles = async (migrationsDirectory, appliedNames) => {
...
//# sourceMappingURL=db-migrations-reset.js.map