...
import { DRIZZLE_SCHEMA_TS, DRIZZLE_SEED_SQL, drizzleConfigTs, SEED_MIGRATION_NAME, STARTER_MIGRATION_NAME, STARTER_MIGRATION_SQL, STARTER_TABLE, } from './util/init-data.js';
import { resolveMigrationsDirectory } from './util/migrations-path.js';
import { hasDependency } from './util/package-json.js';
import { getPackageManager, installPackages } from './util/packages.js';
...
const carefullyWriteFile = async (filePath, data, projectRoot) => {
...
const promptForQueryStyle = async (interactive) => {
    if (!interactive) {
        return 'drizzle';
    }
    log('');
    const { queryStyle } = await inquirer.prompt([
        {
            type: 'list',
            name: 'queryStyle',
...
const promptForStarter = async (interactive) => {
    if (!interactive) {
        return true;
    }
    log('');
...
const installDependencies = async (pm, projectRoot, migrationsDirectory, queryStyle) => {
    ...
    const toInstall = [];
    const confirmedNames = [NETLIFY_DATABASE_PACKAGE];
...
const scaffoldStarter = async (pm, projectRoot, migrationsDirectory, queryStyle) => {
...
const applyAndQuery = async (projectRoot, migrationsDirectory) => {
    log('');
    ...
    try {
        ...
        let query = null;
        try {
            const result = await connection.client.query(`SELECT * FROM ${STARTER_TABLE}`);
            query = {
...
const printNextSteps = (orm, withStarter) => {
    log('');
...
export const initDatabase = async (options, command) => {
    ...
    if (existingMigrations.length > 0) {
        log('');
        info(`It looks like you already have migrations set up in ${chalk.bold(relativeToProject(projectRoot, migrationsDirectory))}.`);
    ...
    info(`Netlify manages and applies migrations for you. Read more at ${chalk.cyan('https://ntl.fyi/database-migrations')}.`);
    info('');
    ...
    if (withStarter) {
        ...
        info('Applying the migration to your local database. This step is handled automatically by Netlify when you deploy.');
