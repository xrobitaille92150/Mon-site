...
export const apiCommand = async (apiMethodName, options, command) => {
    ...
    const apiMethod = isValidApiMethod(api, apiMethodName) ? Reflect.get(api, apiMethodName) : undefined;
    ...
    if (options.data) {
        if (typeof options.data === 'string') {
            try {
                ...
                return logAndThrowError(`Invalid JSON provided to the ${chalk.cyanBright('--data')} flag.
Received: ${received}
...
The ${chalk.cyanBright('--data')} flag must include the path variable(s) required by ${apiMethodName}${requiredNames ? `: ${requiredNames}` : ''}, e.g. --data '{"site_id":"123456"}'`);
        }
