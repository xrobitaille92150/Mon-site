...
For detailed configuration options, see the Netlify documentation.`)
    ...
    .option('--env <KEY=VALUE>', 'Set an environment variable for this deploy only. Applies to deployed functions only. Can be specified multiple times.', parseDeployEnvVar('--env'))
    .option('--secret-env <KEY=VALUE>', 'Set a secret environment variable for this deploy only. Applies to deployed functions only. The value is masked in the Netlify UI and API. Can be specified multiple times.', parseDeployEnvVar('--secret-env'))
    ...
    .addOption(new Option('--create-site [name]', 'Create a new site and deploy to it. Optionally specify a name, otherwise a random name will be generated. Uses your default team if --team is omitted.').hideHelp(true))
    .option('--site-name <name>', 'Name for a new site. Implies --create-site if the site does not already exist.')
    .option('--team <slug>', 'Specify team slug when creating a site. Only works with --create-site or --site-name flag.')
    .option('--allow-anonymous', 'If not logged in, deploy anonymously and create a claimable site instead of requiring authentication', false)
