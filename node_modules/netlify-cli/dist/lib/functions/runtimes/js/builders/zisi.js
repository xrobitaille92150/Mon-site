...
const buildFunction = async ({ cache, config, featureFlags, func, hasTypeModule, projectRoot, targetDirectory, }) => {
    ...
    const { entryFilename, excludedRoutes, includedFiles, inputs, invocationMode, mainFile, outputModuleFormat, path: functionPath, routes, runtimeAPIVersion, schedule, } = await memoize({
        cache,
