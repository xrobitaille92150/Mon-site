...
Client.prototype.enqueue = function (message, callback) {
  ...
  if (this.bufferHolder.buffer.length + messageToAdd.length > this.maxBufferSize) {
    debug('hot-shots enqueue: buffer full (%d + %d > %d), flushing',
      this.bufferHolder.buffer.length, messageToAdd.length, this.maxBufferSize);
    this.flushQueue(callback);
    this.bufferHolder.buffer += (this.bufferHolder.buffer === '' ? '' : '\n') + message;
  }
  else {
    this.bufferHolder.buffer += messageToAdd;
    debug('hot-shots enqueue: added to buffer, new size=%d', this.bufferHolder.buffer.length);
    if (callback) {
      callback();
    }
  }
...
Client.prototype.flushQueue = function (callback) {
  if (this.bufferHolder.buffer && this.bufferHolder.buffer.length > 0) {
    debug('hot-shots flushQueue: flushing %d bytes', this.bufferHolder.buffer.length);
  }
...
Client.prototype.sendMessage = function (message, callback) {
  ...
  try {
    ...
    debug('hot-shots sendMessage: sending %d bytes via %s transport (messagesInFlight=%d)',
      message.length, this.protocol, this.messagesInFlight);
